...
author: 金色传说大聪明
source: 微信公众号
url: https://mp.weixin.qq.com/s?__biz=MzkzNDQxOTU2MQ==&mid=2247519989&idx=1&sn=21192883ca4d44218937fe887fbd37f4&chksm=c313c9360e9f1d77cfb31a1f615986fb29e9878b1fabf68624f86e84964104b3d37ef5dc3391&mpshare=1&scene=1&srcid=0826qyTe8p18zHapJ0KpjZmE&sharer_shareinfo=c1a4b9a3c6172aba6d41a9f95102afb1&sharer_shareinfo_first=c1a4b9a3c6172aba6d41a9f95102afb1#rd
saved: 2026-08-26 13:20:29
tags:
  - 笔记同步助手
id: 0c3789c7-c0a0-47a2-8ec4-b1d39dd6a9b8
---
...
![[笔记同步助手/images/aaf48cc38bf7ac4690a1231711ef33ce_MD5.png]]
...
![[笔记同步助手/images/a9f92eaceb10149e1656c9877dc18df6_MD5.png]]
...
![[笔记同步助手/images/1a123a52c921f3447479eef30375a7ba_MD5.png]]
...
![[笔记同步助手/images/79119b1fa3e26c890ba076b958294bf9_MD5.png]]
...
> plan.md 示例
> \# Plan: claims status self-service (from intent.md 2026-06-02)  ## Files that change portal/src/claims/StatusPanel.tsx (new), claims-api/routes/status.py, claims-api/tests/test\_status.py  ## Order of work 1. Add the status endpoint behind existing auth. 2. Panel against the endpoint. 3. Wire into the portal nav.  ## Risks The claims-core API rate-limits at 50 rps; the panel must cache.  ## Proof test\_status.py covers the four claim states; screenshot matches the approved mock.
...
> name: Agent evals on:   pull\_request:     paths: \['CLAUDE.md', '.claude/\*\*'\]   schedule:     - cron: '0 2 \* \* \*' jobs:   evals:     runs-on: ubuntu-latest     steps:       - uses: actions/checkout@v4       - run: npm install -g @anthropic-ai/claude-code       - name: Run eval suite         env:           ANTHROPIC\_API\_KEY: ${{ secrets.ANTHROPIC\_API\_KEY }}         run: |           for eval in evals/\*.json; do             claude -p "$(jq -r '.prompt' $eval)" \\               --allowedTools "Read,Edit,Bash(make test)" \\               --output-format json > result.json             ./evals/check.sh "$eval" result.json           done
...
> {     "hooks": {       "PreToolUse": \[         {           "matcher": "Bash",           "hooks": \[             { "type": "command",               "command": "${CLAUDE\_PROJECT\_DIR}/.claude/hooks/production-gate.sh" }           \]         }       \]     } }

> .claude/hooks/production-gate.sh
> #!/bin/bash # Production deploys require a named release authorization cmd=$(jq -r '.tool\_input.command' < /dev/stdin) if \[\[ "$cmd" == \*"deploy"\* && "$cmd" == \*"production"\* \]\]; then    if \[ -z "$RELEASE\_APPROVAL" \]; then      echo "Production deploys need a release authorization." >&2      exit 2    fi fi exit 0
...
> metric: ci\_test\_failure\_rate baseline: rolling\_30d rules: western\_electric tiers:   1sigma: { action: log }   2sigma: { action: diagnose,             tools: "Read,Grep,Bash(gh run view \*)" }   3sigma: { action: propose,             routes: \[pull\_request, runbook:rollback-deploy\] }
...
![[笔记同步助手/images/f9521548c9b5609c6557a131c933a544_MD5.png]]
...
内容效果不满意？[点此反馈](https://feedback.notebooksyncer.com/feedback/77f0e902_1787721627956?u=https%3A%2F%2Fmp.weixin.qq.com%2Fs%3F__biz%3DMzkzNDQxOTU2MQ%3D%3D%26mid%3D2247519989%26idx%3D1%26sn%3D21192883ca4d44218937fe887fbd37f4%26chksm%3Dc313c9360e9f1d77cfb31a1f615986fb29e9878b1fabf68624f86e84964104b3d37ef5dc3391%26mpshare%3D1%26scene%3D1%26srcid%3D0826qyTe8p18zHapJ0KpjZmE%26sharer_shareinfo%3Dc1a4b9a3c6172aba6d41a9f95102afb1%26sharer_shareinfo_first%3Dc1a4b9a3c6172aba6d41a9f95102afb1%23rd&s=obsidian)