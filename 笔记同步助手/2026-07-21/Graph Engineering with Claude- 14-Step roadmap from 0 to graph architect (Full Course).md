...
saved: 2026-07-21 23:55:10
tags:
  - 笔记同步助手
id: 0cbf1e05-8555-405d-963c-2c8dfc268748
---

![[笔记同步助手/images/b7229b4b123977b8033eaf5c83d9e979_MD5.jpg|cover]]
...
![[笔记同步助手/images/99c6ef2caf64123fd68e6afc8f1f94be_MD5.jpg]]
...
![[笔记同步助手/images/c007897005e8f22b7c4e9d2d62fe4f92_MD5.png]]
...
\`\`\`python Draw it as boxes and arrows. A box is an agent() call. An arrow is a variable passed from one call’s return into another’s prompt. If you can’t draw the arrow - if no variable crosses - the two boxes are independent, and independence is the thing you’ll exploit for the rest of this course.
...
![[笔记同步助手/images/dafbe089791d37e7098a16959bf4d5d7_MD5.png]]
...
![[笔记同步助手/images/67b8544cadb5b731211ca7a96e1bd274_MD5.jpg]]
...
\`\`\`python // A node with a real contract: bounded in, validated out, one job. const ITEM = { type: 'object', additionalProperties: false, properties: { title: { type: 'string' }, url: { type: 'string' }, impact: { type: 'string', enum: \['high', 'medium', 'low'\] }, }, required: \['title', 'url', 'impact'\], };

const result = await agent(source.prompt, { label: \`research:${source.key}\`, schema: ITEM, // forces validated structured output agentType: 'general-purpose', }); // result is now a shape the next node can trust — not free text. \`\`\`
...
![[笔记同步助手/images/5e259dd2e8e0ae3898201e0a324c94dd_MD5.png]]
...
\`\`\`python The temptation is to spawn an agent to “combine the results.” Resist it. If combining means flatten-and-dedupe, that’s results.flatMap(...) and a Set — deterministic, instant, zero tokens. Save agents for judgment, not for plumbing. A graph where every edge is an agent is a graph paying rent on its own wiring.
...
![[笔记同步助手/images/362396d77824901b58ebe4ffb8c938c0_MD5.jpg]]
...
\`\`\`python phase('Research');

// Nine sources, nine agents, all at once. const raw = await parallel( SOURCES.map((s) => () => agent(s.prompt, { label: \`research:${s.key}\`, phase: 'Research', schema: ITEM\_SCHEMA, // each node returns validated JSON agentType: 'general-purpose', }), ), );
...
![[笔记同步助手/images/b0e5fbae6f19b876bb4ba158a792b6f4_MD5.jpg]]
...
\`\`\`python // The edge: plain JS, no agent, zero tokens. const flat = collected.flatMap((c) => c.items); log(\`Collected ${flat.length} items\`);

phase('Curate'); // The barrier node: needs the WHOLE set to dedupe + rank. const curated = await agent( \`Dedupe and rank these by impact:\\n${JSON.stringify(flat)}\`, { phase: 'Curate', schema: CURATED\_SCHEMA }, ); \`\`\`
...
![[笔记同步助手/images/f5fceeeb2a9e292d91bac221114d23ea_MD5.png]]
...
![[笔记同步助手/images/2d3120e4c30507c2bc0354bdbb5fe10b_MD5.png]]
...
\`\`\`python // Router node: an agent classifies, code picks the edge. const { severity } = await agent( \`Classify this diff's risk:\\n${diff}\`, { schema: { type: 'object', properties: { severity: { enum: \['low', 'high'\] } }, required: \['severity'\] } }, );

let review; if (severity === 'high') { // heavy path: full parallel audit review = await parallel(FILES.map((f) => () => agent(\`Audit ${f}\`))); } else { // light path: one quick pass review = await agent(\`Quick review of ${diff}\`); } \`\`\`
...
![[笔记同步助手/images/9d35f770c4162b3854f3600ad8293a5a_MD5.png]]
...
![[笔记同步助手/images/8beff25e0966feb5bee90ff111ddc198_MD5.png]]
...
![[笔记同步助手/images/f4ad1ace5d67df81ee0b0c1291473cd4_MD5.png]]
...
while (dry < 2) { // stop after 2 empty rounds const found = (await parallel( FINDERS.map((f) => () => agent(f.prompt, { schema: BUGS })) )).filter(Boolean).flatMap((r) => r.bugs);

const fresh = found.filter((b) => !seen.has(key(b))); if (!fresh.length) { dry++; continue; } // nothing new → toward dry dry = 0; fresh.forEach((b) => seen.add(key(b))); // dedupe vs SEEN, not confirmed

// diverse-lens verify each fresh finding before it counts const judged = await parallel(fresh.map((b) => () => parallel(\['correctness', 'security', 'repro'\].map((lens) => () => agent(\`Judge "${b.desc}" via ${lens} — real?\`, { schema: VERDICT }))) .then((v) => ({ b, real: v.filter(Boolean).filter((x) => x.real).length >= 2 }))));
...
![[笔记同步助手/images/bcc9fb5513f6be4d4ac348000936e04f_MD5.jpg]]
...
![[笔记同步助手/images/30bd8aeb70645cf2c7e3c35c834477ba_MD5.png]]
...
![[笔记同步助手/images/312fbe5d9a58f983baddaf2d9b32585e_MD5.jpg]]
...
内容效果不满意？[点此反馈](https://feedback.notebooksyncer.com/feedback/645591b4_1784649308116?u=https%3A%2F%2Fx.com%2F0xCodez%2Fstatus%2F2079165300625330317)